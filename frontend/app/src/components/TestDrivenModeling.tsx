import { moddleToDCR, runTest, type Trace } from "dcr-engine";
...
const TestWindow = styled.div<{ $embedded: boolean }>`
...
const TestResultsWindow = styled(ResultsWindow)<{ $embedded: boolean }>`
  position: ${(props) => (props.$embedded ? "absolute" : "fixed")};
  height: ${(props) => (props.$embedded ? "100%" : "100vh")};
`;
...
const resultIcon = (val: boolean | undefined) => {
  switch (val) {
    ...
    case true:
      return (
        <BiLike title="Test Passed" style={{ backgroundColor: "green" }} />
      );
    case false:
      return (
        <BiDislike title="Test Failed" style={{ backgroundColor: "red" }} />
      );
  }
};

const TestDrivenModeling = ({ modeler, show, embedded = false }: TestDrivenModelingProps) => {
  ...
  const downloadTests = () => {
    ...
    Object.keys(testsToWrite).forEach(
      (key) => (testsToWrite[key].result = undefined)
    );
  ...
  return (
    <>
      {creatingTest && (
        <Popup close={() => setCreatingTest(false)}>
          <CreateTest>
            <Form
              submit={(formData: FormData) => {
                ...
                const graph = moddleToDCR(modeler.getElementRegistry(), true);
            ...
            >
              ...
              Trace
              <textarea
                title="The partial trace of the test. Input one activity per line."
                name="trace"
                required
                defaultValue={currentTest.trace}
              />
              Context
              <textarea
                title="The context of the test. Input one activity per line."
                name="context"
                required
                defaultValue={currentTest.context}
              />
              Polarity
              <Input
                title="The polarity of the test. Checked means a positive test case."
                name="polarity"
                type="checkbox"
                defaultChecked={true}
              />
      ...
      {show && (
        <TestResultsWindow $embedded={embedded} $traceSelected={selectedTest !== null}>
          <EventLogInput
            value={name}
            onChange={(e) => setName(e.target.value)}
          />
          {Object.values(tests).map(
            ({ trace, testName, testId, context, polarity, result }) => (
              <ResultsElement
                $selected={
                  selectedTest !== null && selectedTest.testId === testId
                }
                key={testId}
                onClick={() => {
                  setSelectedTest({
                ...
                {resultIcon(result)}
                {
                  <DeleteTest
                    onClick={(e) => {
                      ...
                      if (
                        confirm(
                          `This will delete the test '${testName}'. Are you sure?`
                        )
                      ) {
                        const newTests = { ...tests };
          ...
          >
            <FlexBox direction="row" $justify="space-around">
              <Button
                onClick={runTests}
                title="Length of traces to check test against. 0 for infinite length."
                style={{ margin: "1rem" }}
              >
                Run tests to depth:
              </Button>
              ...
              <Button
                onClick={() => {
                  ...
                  const exampleEvents = [...graph.events]
                    .slice(0, 3)
                    .map((event) => graph.labelMap[event]);
      ...
      {selectedTest && (
        <TestWindow $embedded={embedded}>
          ...
          Trace:
          <ul>
            {selectedTest.trace.map((event, idx) => (
              <Activity key={event + idx}>{event}</Activity>
            ))}
          </ul>
          Context:
          <ul>
            {[...selectedTest.context].map((event, idx) => (
              <Activity key={event + idx}>{event}</Activity>
            ))}
          </ul>
        </TestWindow>
