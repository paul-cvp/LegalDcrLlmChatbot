...
import { copyMarking, type DCRGraph, moddleToDCR } from "dcr-engine";
...
interface ModelerProps {
  ...
  override?: {
    graphRef: React.RefObject<
      | { initial: DCRGraph; current: DCRGraph }
      | { initial: DCRGraphS; current: DCRGraphS }
      | null
    >;
    overrideOnclick: (e: unknown) => void;
...
function LegacyModeler({
  ...
  useEffect(() => {
    ...
    if (!modelerRef.current) {
      ...
      initModeler
        .importXML(initXml ? initXml : emptyBoardXML)
        .then(() => {
          modelerRef.current = initModeler;
          if (override) {
            const graph = moddleToDCR(modelerRef.current.getElementRegistry());
            override.graphRef.current = {
              initial: graph,
              current: { ...graph, marking: copyMarking(graph.marking) },
            };
            ...
            try {
              if (override.onLoadCallback) {
                override.onLoadCallback(graph);
              }
            } catch (e) {
              console.log(e);
            }
  ...
  useEffect(() => {
    if (modelerRef.current) {
      modelerRef.current.setSetting("blackRelations", !coloredRelations);
    }
  }, [coloredRelations]);

  useEffect(() => {
    if (modelerRef.current) {
      modelerRef.current.setSetting("markerNotation", markerNotation);
    }
  }, [markerNotation]);

  return (
    <div
      className={override?.canvasClassName ? override.canvasClassName : ""}
      id="canvas"
    />
  );
}

export default LegacyModeler;
