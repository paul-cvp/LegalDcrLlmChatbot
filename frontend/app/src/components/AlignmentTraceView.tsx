...
import type { RoleTrace, Trace } from "dcr-engine/src/types";
...
const AlignmentTraceView = ({
  selectedTrace,
  setSelectedTraceId,
  onCloseCallback,
}: AlignmentTraceViewProps) => {
  return (
    <TraceWindow>
      <ResultsHeader>
      ...
      {selectedTrace.results && selectedTrace.results.cost !== 0 && (
        <>
          <ResultsHeader>Aligned:</ResultsHeader>
          <ul>
            {selectedTrace.results.trace.map((event, idx) => (
              <Activity key={event + idx + selectedTrace.trace.length}>
                {event}
              </Activity>
            ))}
          </ul>
