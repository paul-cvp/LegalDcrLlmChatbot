...
export default function LabelEditingProvider(
  ...
  modeling, resizeHandles, textRenderer, dcrRules) {
  ...
  function decideIfTitelOrAttributesClicked(event) {
    ...
    if (click_y_coordinate >= titel_attribute_divider_y_coordinate) {
      event.element.businessObject.labelAttribute = 'label';
    } else {
  ...
  eventBus.on('element.dblclick', function (event) {
    if (isAny(event.element, ['dcr:SubProcess'])) {
      event.element.businessObject.labelAttribute = 'label';
    } else {
  ...
  function activateDirectEdit(element, force) {
    if (dcrRules.canHaveLabel(element) &&
      (force ||
        isAny(element, ['dcr:TextBox', 'dcr:Event']))) {
...
LabelEditingProvider.prototype.getEditingBBox = function (element) {
  ...
  if (isAny(element, ['dcr:Nesting', 'dcr:Event', 'dcr:SubProcess'])) {
    ...
    if (isAny(element, ['dcr:Event', 'dcr:Nesting'])) {

      // Editing attributes should be different.
      if (element.businessObject.labelAttribute === 'label') {
        assign(bounds, {
          y: bbox.y + (30 * zoom),
