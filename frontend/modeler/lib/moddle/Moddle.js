...
DCRModdle.prototype.toXML = function (element, options) {
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      return resolve({
        // Keep expression attributes stable across frontend and backend exporters.
        xml: result.replace(/\b(guard|value)="([^"]*)"/g, function(_, name, value) {
          return name + '="' + value.replace(/&#(?:0*62|x0*3e);/gi, '&gt;') + '"';
        })
      });
    } catch (err) {
      return reject(err);
