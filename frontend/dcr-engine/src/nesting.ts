import type { DCRGraph, Event, EventMap, Marking, RelationType } from "./types";
import {
  copyMarking,
  flipEventMap,
  customIntersect,
...
function relationsDictToDCR(
  ...
  const stringToRelation = (str: RelationType): EventMap => {
    switch (str) {
      case "condition":
        return graph.conditionsFor;
      case "exclude":
        return graph.excludesTo;
      case "response":
        return graph.responseTo;
      case "include":
        return graph.includesTo;
      case "milestone":
        return graph.milestonesFor;
      default:
        throw new Error(`Unsupported relation type for automatic nesting: ${str}`);
    }
  };
  ...
  for (const event in relationsDict) {
    for (const relation of relationsDict[event]) {
      ...
      if (substrings.length < 3)
        throw new Error("I didn't implement for this, easy fix though. ggez.");
      const otherEvent =
        substrings.length === 3
          ? substrings[0]
          : substrings.slice(0, -2).join("_");
...
function getOppositeRelDictStr(
  relationDictStr: string,
  event: Event,
  nestingId: string
): {
  event: string;
  relationDictStrDel: string;
  relationDictStrAdd: string;
} {
  const substrings = relationDictStr.split("_");
  if (substrings.length < 3) throw new Error("This shouldn't happen");
  const otherEvent =
    substrings.length === 3 ? substrings[0] : substrings.slice(0, -2).join("_");
  const relStr = substrings[substrings.length - 2] as RelationType;
  const direction = substrings[substrings.length - 1] as "out" | "in";
...
function setToStr(set: Set<string>) {
  return Array.from(set).sort().join();
}

function getRelationsDict(graph: DCRGraph): RelationsDict {
...
export function nestDCR(
  graph: DCRGraph,
  minSharedRels: number = 1,
  events = new Set(graph.events),
  relationDict: RelationsDict = getRelationsDict(graph)
): ReturnNesting<DCRGraph> {
  let idCounter = 0;
  ...
  const findLargestNesting = (
    events: Set<Event>,
    parentNesting?: { id: string; events: Set<Event> }
  ): Set<Event> => {
    ...
    for (let i = 0; i < eventArr.length; i++) {
      for (let j = i + 1; j < eventArr.length; j++) {
        ...
        const shared = customIntersect(relationDict[e1], relationDict[e2]);
        if (!possibleNestings[setToStr(shared)]) {
          possibleNestings[setToStr(shared)] = {
            nestingEvents: new Set([e1, e2]),
            sharedRels: shared,
          };
        } else {
          mutatingUnion(
            possibleNestings[setToStr(shared)].nestingEvents,
            new Set([e1, e2])
          );
        }
      }
    ...
    let bestNesting = {
      nestingEvents: new Set<string>(),
      sharedRels: new Set<string>(),
    };
    let max = 0;
    for (const possibleNestingId in possibleNestings) {
      ...
      if (
        parentNesting &&
        mutatingDifference(new Set(parentNesting.events), nesting.nestingEvents)
          .size === 0
      )
        continue;
    ...
    if (
      !(
        bestNesting.nestingEvents.size > 1 &&
        bestNesting.sharedRels.size >= minSharedRels
      )
    )
      return new Set();
    ...
    if (parentNesting) {
      mutatingDifference(parentNesting.events, bestNesting.nestingEvents);
    ...
    for (const nestedEvent of bestNesting.nestingEvents) {
      mutatingDifference(relationDict[nestedEvent], bestNesting.sharedRels);
      for (const sharedRelStr of bestNesting.sharedRels) {
        const { event, relationDictStrDel, relationDictStrAdd } =
          getOppositeRelDictStr(sharedRelStr, nestedEvent, nestingId);
    ...
    relationDict[nestingId] = new Set(bestNesting.sharedRels);
    ...
    for (const nestedEvent of bestNesting.nestingEvents) {
      if (!stillSharedRels) {
        ...
        stillSharedRels = customIntersect(
          stillSharedRels,
          relationDict[nestedEvent]
        );
      }
    }
    ...
    const eventsToConsider = new Set(bestNesting.nestingEvents);
    const allEventsInNesting = new Set(bestNesting.nestingEvents);
    let repeat = true;
    while (repeat) {
      const largestNesting = findLargestNesting(eventsToConsider, {
        id: nestingId,
        events: bestNesting.nestingEvents,
      });
      mutatingDifference(eventsToConsider, largestNesting);
  ...
  while (repeat) {
    ...
    mutatingDifference(events, eventsFound);
    repeat = eventsFound.size !== 0;
  }

  return {
    nestedGraph: {
      ...relationsDictToDCR(relationDict, graph.marking),
      events: mutatingUnion(new Set(graph.events), nestingIds),
    },
    nestingIds,
    nestingRelations,
  };
