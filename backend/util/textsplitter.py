...
from .localembeddings import get_local_embedding_model
...
def _token_count(text: str) -> int:
    return get_local_embedding_model().count_tokens(text)
...
class SentenceTextSplitter(TextSplitter):
    ...
    def split_page_by_max_tokens(self, page_num: int, text: str) -> Generator[Chunk, None, None]:
        ...
        """
        if _token_count(text) <= self.max_tokens_per_section:
    ...
    def _append_overlap(self, prev_chunk: Chunk, next_chunk: Chunk) -> Chunk:
        ...
        if len(candidate) > max_chars or _token_count(candidate) > self.max_tokens_per_section:
            # Attempt to shrink prefix at word / sentence boundaries from its start
            shrink = prefix
            while shrink and (
                len(prev_chunk.text + shrink) > max_chars
                or _token_count(prev_chunk.text + shrink) > self.max_tokens_per_section
            ):
                cut_index = 1
        ...
        for page in pages:
            ...
            if previous_chunk and page_chunks:
                ...
                if (
                    ...
                    if _token_count(combined_text) <= self.max_tokens_per_section and len(combined_text) <= int(
                        ...
                        if fragment_start < len(prev_text):
                            ...
                            def fits(candidate: str) -> bool:
                                ...
                                if len(combined) > max_chars:
                                    return False
                                if _token_count(combined) > self.max_tokens_per_section:
                                    return False
                                return True
                            ...
                            if not fits(move_fragment):
                                ...
                                while (
                                    move_fragment
                                    and _token_count(move_fragment + first_new_text) > self.max_tokens_per_section
                                ):
                                    move_fragment = (
